...
uniform sampler2D uSSFRRefractionTextureBlurX;
...
void main() {
	...
	float blurredDepth = blurFragmentBilateral(vTextureCoord, shift, uSSFRDepthTextureBlurX);

	float blurredRefraction = 0.0;

	if(!uDisableColorMixing)
		blurredRefraction = blurFragmentBilateral(vTextureCoord, shift, uSSFRRefractionTextureBlurX);
